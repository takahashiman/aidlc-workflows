...
### 🔖 RESUME POINT（次セッション開始点・2026-06-25 更新・シナリオ①-補遺ページ設計完了＋デバイスコンテキスト判定層追加＋形態最適化フロー（手順3）コンテキスト別タブ化→形態最適化ガイド完成）
- **いま（最新・2026-06-25 実装完了・シナリオ①-補遺「コンポーネント形態の最適化」ページ完成＋デバイスコンテキスト判定層統合＋形態最適化フロー（手順 3）のコンテキスト別タブ化・未コミット／PR #11 候補・main ブランチへの merge 待ち）**: 
  
  **進捗経過**: ① T3（Core import 準備確認）設計・実装・検証完了（2026-06-24 昼）→ portal vitest 42/42・build PASS。② T4（置換完了判定チェック）設計案を T3 と同じ AI/人間分離パターンで実装（2026-06-24 夜）→ portal vitest 42/42・build PASS。③ Haiku が那覇空港サイネージで置換実行を完了 → T4 自動チェックで「全項目 ✅ PASS・**置換完全性 100%**」と報告。④ **シナリオ①-補遺「コンポーネント形態の最適化」ページ設計・実装完了**（2026-06-24 深夜）→ T5_FORM_AI_TEXT・T5_FORM_CHECK_TEXT・FORM_OPTIMIZATION_RULES・CH_FORM_OPTIMIZATION を作成・portal vitest 42/42・build PASS。⑤ **デバイスコンテキスト判定層を形態最適化フローに統合**（2026-06-25）→ Core DS に Large Display フォントスケール（36-72px）追加・T5_DEVICE_CONTEXT_CHECK / T5_FORM_AI_TEXT_LD テンプレ追加・ポータル 6.5 章修正完了・portal vitest 42/42・build PASS・memory 新規作成 [[device-context-typography]]。⑥ **形態最適化フロー（手順 3）のコンテキスト別タブ化**（2026-06-25）→ FORM_OPTIMIZATION_RULES を Web/Mobile / Large Display / Hybrid の 3 つに分割・各コンテキストのスケール・ルール・具体例を詳細化・ポータル手順 3 を tabs で 3 コンテキスト選択可能に実装・dev-flow-journal.md に Step 9 補遺 2 を記録・portal vitest 42/42・build PASS。⑦ **テンプレート形式の統一＋手順 3/4 分離注記**（2026-06-25）→ T5_FORM_INSTRUCTION_EXAMPLE テンプレート変数定義・修正指示を k: 'ai' 形式で統一（他テンプレと同じく変数参照化）・FORM_OPTIMIZATION_RULES_* 全 3 つに「本テンプレートの出力範囲（手順 3）」注記追加・LLM が手順 4 領域に越界するのを防止＋ポータル形式一貫性確保・dev-flow-journal.md に Step 9 補遺 3 を記録・portal vitest 42/42・build PASS。⑧ **修正スコープ自動確定フロー（手順 3.5 新設）**（2026-06-25）→ 人間の判定負荷削減＋コンテキスト依存実現 のため、「AI が診断結果から修正パターン（❶全て統一可 / ❷標準寄り / ❌スケール混在）を自動判定する手順 3.5」を新設・T5_FORM_SCOPE_CONFIRMATION テンプレ定義・修正スコープ確定表を自動生成・人間は確認するだけ・ポータル手順 3→3.5→4 の流れを完成・dev-flow-journal.md に Step 9 補遺 4 を記録・portal vitest 42/42・build PASS。
  ...
  **次セッション予定（形態最適化の実行フェーズ）**: 
  ...
  1. T5_DEVICE_CONTEXT_CHECK でデバイスコンテキスト判定（Web/Mobile / Large Display / Hybrid）
  2. T5_FORM_AI_TEXT / T5_FORM_AI_TEXT_LD で現状診断（ボタン・カード・テキストの形態を計測・列挙）
  3. T5_FORM_CHECK_TEXT で Core 標準との比較
  4. FORM_OPTIMIZATION_RULES（該当コンテキスト版）で判定ルール確認
  5. **[新] T5_FORM_SCOPE_CONFIRMATION で修正スコープを自動確定** ← AI が診断結果からパターンマッチングして修正スコープ確定表を生成
  6. T5_FORM_INSTRUCTION_EXAMPLE で修正指示を生成・実装
  7. VRT・a11y テスト確認・非回帰確認
  → 「自社デザイン資産化」完了判定
  
  - **変更ファイル（✅ コミット完了）**: 
    - `portal/vendor/core/primitives.css`（✅ **Large Display フォントスケール追加完了** --font-size-ld-sm～2xl 36-72px・構文検証通過）
    - `portal/src/usage.js`（✅ T4_AI_TEXT・T4_CHECK_TEXT・章6 実装・**新規：T5_FORM_AI_TEXT・T5_FORM_CHECK_TEXT・T5_DEVICE_CONTEXT_CHECK・T5_FORM_AI_TEXT_LD・FORM_OPTIMIZATION_RULES_WEB_MOBILE・FORM_OPTIMIZATION_RULES_LD・FORM_OPTIMIZATION_RULES_HYBRID・T5_FORM_SCOPE_CONFIRMATION・T5_FORM_INSTRUCTION_EXAMPLE** + **手順 3 タブ化**（コンテキスト別判定ルール） + **手順 3/4 分離注記**（各 FORM_OPTIMIZATION_RULES_* に出力範囲を明記） + **手順 3.5 新設**（修正スコープ自動確定・診断結果からパターンマッチング））
...
- **いま（最新・2026-06-23 マージ＆ライブ公開済）**: ポータル UX 精緻化＋Core リンク是正を **全 PR マージ＆ライブデプロイ完了**。公開先 **https://takahashiman.github.io/aidlc-workflows/**（portal-deploy run 28007739974＝build/quality/deploy 全 success）。
  ...
  - **🆕 将来タスク（2026-06-23 記録・今回保留）＝未整備プレビュー 26 件の一括整備**: Live Preview「未収録」のコンポーネントが計 26 件（実体ファイル無し 22＋`preview:null` 4・整備済 26）。**Badge もこの 1 つ**（CSS `.fig-badge`/spec は在るが `preview/components-badge.html` 未作成→ポータルが余白として正直に表示）。U2-6 arrival-card 型の余白解消（Core に preview HTML 新規・生HEX0・AA→core マージ→rolling 反映）を一括で行うタスクが将来必要。Badge 整備時は portal-content.js の code 例（`badge`→`.fig-badge`）と spec の Preview 誤記も是正。一覧・原因は memory `portal-uncovered-previews`＋audit（2026-06-23 Badge 調査）。**最優先目標が別のため今回は記録のみ。**[[portal-uncovered-previews]]
...
## Current Status
- **2026-06-27 palette-gen に N 色相アクセント生成を追加（relation 駆動・greenfield／リブランド向け・post-U2/U7 polish）**: 単一 seed＝ドミナント（monochromatic）だった生成器を、**色相回転による調和アクセント派生**へ拡張。**①N 色相化**＝新 LC `tools/palette-gen/accent-deriver.mjs`（`SCHEMES`：monochromatic / dyad / triad / tetrad / pentad / **hexad（最大六色）** / analogous / complementary / split-complementary。n-adic は seed H から 360/N 間隔）。**②二段構成**＝「L 統一（accent.base は seed の L・C を保持し H のみ回転＝調和の制約）」＋「AA 補正（各アクセントは `deriveSignature` を通し on-color を ensureOnColor で AA 保証＝上掛け。OKLCH の L と WCAG 輝度 Y は不一致のため L 統一だけでは AA 不可）」。**実装**＝`index.mjs`（generatePalette に `scheme` 追加・accents 導出・css.accent 生成・verifyInvariants が accent.on/dark も検査）／`emitter.mjs`（`emitAccentCss`：`--accent-{n}-*`＋accent-1 を `--color-brand-secondary` へ委譲＝signature が brand-primary を再テーマするのと対称）／`generate.mjs`（`--scheme=` CLI・`accent.css`/`accent-aaa.css` は **scheme≠monochromatic 時のみ出力**＝既定はバイト不変で drift/後方互換維持）。**境界**＝既存製品の配色は**自動判別しない**（意図を頻度/幾何から捏造しない）。scheme は宣言入力。**③プロンプト分岐**＝`usage.js` の移行 AI プロンプト（`CH5_AI_TEXT`）の ❶ を「❶-A 保持（製品ローカル化・ブランド不変）／❶-B リブランド（`--scheme` で accent 生成し `--accent-{n}-*`/`--color-brand-secondary` へ寄せる）」に分岐（陳腐化した「seed 1色からしか作らない」を撤去）。`ai-co-creation.js`（greenfield）手順3に `--scheme` の任意指定＋accent 利用を追記。**CI**＝`lint-rules.json` の `layers.semantic` に `**/tokens/accent*.css` を追加（`**/generated/**` は 2-a で対応済）。**検証**＝Core `test:palette` **18 pass**（既存10＋新規8：色相回転=seed H+offset／L 統一±0.06／全 accent on/dark AA／hexad=6色上限／決定性／monochromatic 後方互換／未知 scheme fail-fast／出力契約）・`gen:palette --check` **drift なし**（既定 monochromatic 不変）・split-complementary 実生成で accent.css 確認・lint 回帰なし（既存 1 error のみ）・portal `npm test` **42 pass**・portal JS `node --check` OK。**commit/push はユーザー承認後**。
- **2026-06-26 signature 再テーマ機構の用語統一（Model A→B）＋ generated/ 規約の CI 確定（post-U2/U7 polish）**: signature/status 生成物の「層」と「再テーマ手順」を **Model B（seed→決定的生成→参照）** へ統一し、生成物の置き場規約（`**/generated/**`）を CI lint で確定。**起点**＝那覇案件で `palette-gen` 試行時の「生成 CSS は専用 primitive.css か？」。**正体の確定**＝生成物 `signature.css`/`status.css` は Core lint で **semantic 層**／生 primitive は seed の `--color-signature` ただ1つ（`allowRawColorVars`＝事実上の primitive）＝「丸ごと primitive.css」ではなく「seed＋それを a11y AA 保証で再マップした semantic 色層」（`primitives.css` は再生成せず共有・製品は **component 層のみ自作**・`semantic.css` は Core 継承）。**不整合2件**＝(A) `portal/src/ai-co-creation.js` だけが Model A（`extensions.css` に `--color-signature` 手書き→`color-mix()` 自動派生）を指示も、実体は Model B（OKLCH ビルド時に静的 hex 焼込・`color-mix()` 不使用・`--color-signature` 自己定義）＝手書きで light/dark/tint/shadow が追従せず壊れたパレット／(B) 生成物を `tokens/` 外（`src/styles/generated` 等）へ出すと `**/styles/**`＝component(L3) 扱いで焼込 hex が `V1_raw_hex` ブロック。**変更**＝**1-a** `ai-co-creation.js` を Model B 化（手順3「palette-gen 生成」新設・index.html 読込を `./styles/generated/{signature,status}.css`・`--color-signature` 手書き指示撤去・§シグネチャー取り扱い全面書換・実装ルール1→7連番・`--out styles/generated` 整合）／**1-b** `usage.js` に「生成物 DO NOT EDIT・色変更=seed 再生成」一文追加／**2-a** 正典 Core `FIG-Universal-Design-System/ci/lint/lint-rules.json` の `layers.semantic` に `**/generated/**` 追加＋`_layers_comment` に generated/ 規約明記（出力先非依存で semantic 免除）。**根拠**＝Model B が正典（`BR-PAL-5/7` a11y AA 必達・CI drift 決定性／`color-mix()` 実行時派生は on-color 自動選択・AA 補正・決定性を喪失）。**検証**＝`node --check`（ai-co-creation/usage OK）・lint-rules JSON OK・Core lint スモーク（`generated/` 配下生 hex→**免除**／配下外→**V1_raw_hex 検出**）・V3 は `--signature-*` が primitiveSource 外ゆえ `var(--signature-base)` を誤検知せず（`three-layer-lint.mjs:77-80` 先勝ち primitive→semantic→component 確認）。**注意**＝正典 Core のみ編集（`vendor/core` コピーは submodule 同期で反映・手編集しない）・既存 V3 error（`tokens/components.css:40` の `--color-brand-primary` 直参照）は本作業と無関係。**未決**＝コンポーネント第3層の実体ブラッシュアップは別フェーズ（memory `t4-completion-component-next`/`portal-uncovered-previews` 起点）。**commit/push はユーザー承認後**。
