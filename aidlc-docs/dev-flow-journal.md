...
### Step 9 補遺 3. テンプレート形式の統一＋手順の分離注記（2026-06-25）

**背景**：Step 9 で実装した形態最適化フロー（手順 0-4）のテンプレートについて、
Haiku が自動的にフェーズ分類を生成したことから、**テンプレート設計の曖昧性**が露見。

- **何を**：
  1. 修正指示テンプレート（旧 `k: 'cmd'` inline body）を、他のテンプレートと同じく独立変数化
  2. 「手順 3（判定）と手順 4（実装計画）」の分離を、ポータル上で明記
  3. LLM が越界しないよう、テンプレート出力範囲を注記化

- **Why**：
  - **テンプレート形式の一貫性**：T5_FORM_CHECK_TEXT / T5_FORM_AI_TEXT_LD と同じく
    生成AI形式（`k: 'ai'`）で提示 → ポータルでの表示・スタイル統一
  - **手順分離の明確化**：
    - 手順 3 出力 = 各項目の修正必要性・方針（❶YES / ❷YES / ❌必須）
    - 手順 4 入力 = 人間が複数項目の修正計画（フェーズ・優先度・依存関係）を判定
    - → Haiku が手順 4 の仕事を自動生成するのを防止

- **How**：
  1. `portal/src/usage.js` で `T5_FORM_INSTRUCTION_EXAMPLE` テンプレート変数を新規定義
  2. `FORM_OPTIMIZATION_RULES_WEB_MOBILE` / `FORM_OPTIMIZATION_RULES_LD` / `FORM_OPTIMIZATION_RULES_HYBRID`
     の 3 つ全てに「【本テンプレートの出力範囲（手順 3）】」注記を追記
  3. ポータル Ch6.3「形態最適化の判定ルール」を「修正指示の例」から `T5_FORM_INSTRUCTION_EXAMPLE` 参照に変更（形式統一）

- **確認**：
  - portal `npm run build` 成功
  - portal `npm run test` 42/42 PASS
  - git diff で修正内容（テンプレ分離・形式統一・注記追加）確認

- **つまずき・注意**：
  - ポータルテンプレートが「可視化の場」として機能するまで、LLM は「次は何をするべきか」を推測して越界しやすい
  - 手順分離を **テンプレートの注記** だけでなく、呼び出し側（AI プロンプト）でも明示すると、更に安全

- **ポータル反映**：ユーザーへの実装手順としてはここまで自動反映。
  実際の修正実施（Phase A/B/C）時に「手順 4 の人間判断」がもう一度現れる。

- **関連メモリ更新**：
  - `initiative2-resume-point.md`（次セッション再開点）
  - ポータル状態 = 手順 0-4 まで完全実装・テンプレート形式統一・LLM 越界防止準備完了

---

### Step 9 補遺 4. 修正スコープ自動確定フロー（手順 3.5 新設）（2026-06-25）

**背景**：ユーザーからの指摘「修正指示テンプレが画一的。実プロジェクトの診断結果に基づいて、コンテキスト・修正対象が異なるべき」
→ テンプレート形式統一だけでは不十分。**人間の判定負荷を減らしつつ、コンテキスト依存を実現する仕組み**が必要。

- **何を**：形態最適化フロー（手順 0-4）に**手順 3.5「修正スコープ自動確定」**を新設
  1. AI が診断結果から **3 パターン（❶全て統一可 / ❷標準寄り / ❌スケール混在）** を自動判定
  2. コンポーネントごとの **修正スコープ確定表** を自動生成
  3. 人間は「確定表を確認」するだけ・判定ロジックの自動化で負荷 ↓

- **Why**：
  - **人間の判定負荷削減**：従来は人間が ❶❷❌ ルールを「手動で」適用。パターン判定を AI が自動化。
  - **コンテキスト依存の実現**：修正指示が「このプロジェクトの診断に合わせた」内容に
  - **「画一的でない」を解決**：テンプレ形式統一だけでなく、生成内容を診断結果に依存させる

- **How**：
  1. **T5_FORM_SCOPE_CONFIRMATION テンプレ** を新規定義（portal/src/usage.js）
     - パターン 1: 複数サイズ存在・全て統一可能 → Core トークン参照へ置換
     - パターン 2: 複数パターン・標準寄りあり → 標準寄りに統一
     - パターン 3: Web/Mobile スケール混在 → 全て固定 px 化 → Core 参照へ置換
     - 出力: 修正スコープ確定表（コンポーネントごとの修正内容をリスト化）
  
  2. **ポータル構成の更新**
     - 従来: 手順 2 → 手順 3 → 手順 4
     - 新規: 手順 2 → 手順 3 → [新] 手順 3.5 → 手順 4
     - 手順 3.5: T5_FORM_SCOPE_CONFIRMATION（AI 自動パターンマッチング）
     - 手順 4: T5_FORM_INSTRUCTION_EXAMPLE（修正スコープ確定表を入力とした修正指示生成）

- **確認**：
  - portal `npm run build` ✓
  - portal `npm run test` 42/42 PASS ✓
  - 新規テンプレ T5_FORM_SCOPE_CONFIRMATION の構文検証成功
  - ポータル手順 3.5→4 の連携確認（説明文も更新・スコープ確定表に基づいて修正指示へ）

- **つまずき・注意**：
  - **パターンマッチングのロジック化**：ユーザーの提案が「診断結果を見て AI が自動判定する」という形式だった
    →「テンプレ内に判定ロジック（❶❷❌パターン）を記述し、AI に読ませる」アプローチで対応
  - **「確定表の確認」が手間になるリスク**：完全自動化ではなく「AI が案を出す→人間が確認」フロー
    →「確定表の内容に違和感がないか」という軽い確認で十分（❶❷❌ 判定の負荷なし）

- **ポータル反映**：形態最適化フロー（章 6.5）の手順 0-4 が完全に揃った
  - 手順 0: デバイスコンテキスト判定（T5_DEVICE_CONTEXT_CHECK）
  - 手順 1: 診断（T5_FORM_AI_TEXT / T5_FORM_AI_TEXT_LD）
  - 手順 2: 確認チェック（T5_FORM_CHECK_TEXT）
  - 手順 3: 判定ルール（FORM_OPTIMIZATION_RULES_WEB_MOBILE / LD / HYBRID）
  - **[新] 手順 3.5: スコープ自動確定（T5_FORM_SCOPE_CONFIRMATION）**
  - 手順 4: 修正指示＋テスト（T5_FORM_INSTRUCTION_EXAMPLE）

- **次セッション予定**：実開発リポジトリ（那覇空港サイネージ or BusDelayAlerts）で形態最適化フローを実行
  1. T5_DEVICE_CONTEXT_CHECK でコンテキスト判定
  2. T5_FORM_AI_TEXT / T5_FORM_AI_TEXT_LD で診断実行
  3. T5_FORM_CHECK_TEXT で確認
  4. FORM_OPTIMIZATION_RULES で判定ルール確認
